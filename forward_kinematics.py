...
model = FCN_FWD(input_size=6, output_size=3, dropout_rate=0.2).to(device)  # Move model to GPU if available
criterion = nn.MSELoss()  # Mean Squared Error (MSE) for regression
optimizer = optim.Adam(model.parameters(), lr=0.0001, weight_decay=5e-5)  # L2 Regularization

# Training settings
num_epochs = 1500  # Number of training epochs
best_val_loss = float('inf')  # Track best validation loss
early_stop_patience = 50  # Stop training if no improvement in X epochs
...
for epoch in range(num_epochs):
    ...
    for xyz_batch, joint_batch in train_loader:
        ...
        output = model(joint_batch)  # Forward pass
        loss = criterion(output, xyz_batch)  # Compute loss
    ...
    with torch.no_grad():  # No need to compute gradients during validation
        for xyz_batch, joint_batch in val_loader:
            ...
            output = model(joint_batch)
            loss = criterion(output, xyz_batch)
            val_loss += loss.item()
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), "best_model_fwd.pth")  # Save best model
    else:
...
plt.show()
'''
model.load_state_dict(torch.load("best_model_fwd.pth"))
model.eval()  # Set model to evaluation mode (Dropout OFF)
...
q_sample = q_sample.reshape(1, -1).to(device)
predicted = model(q_sample)
print("The predicted Angles are:",predicted)
print("The real Angles are:",xyz_sample)