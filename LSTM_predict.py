...
model = LSTMNetwork(input_size=3, output_size=6).to(device)  # Move model to GPU if available
criterion = nn.MSELoss()  # Mean Squared Error (MSE) for regression
optimizer = optim.Adam(model.parameters(), lr=0.002, weight_decay=1e-6)  # L2 Regularization
...
for epoch in range(num_epochs):
    ...
    for xyz_batch, joint_batch in train_loader:
        ...
        loss = criterion(output, joint_batch)  # Compute loss
    ...
    with torch.no_grad():  # No need to compute gradients during validation
        for xyz_batch, joint_batch in val_loader:
            ...
            loss = criterion(output, joint_batch)
            val_loss += loss.item()
