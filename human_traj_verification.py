...
model = FCNN_PC(7).to(device)

model.load_state_dict(torch.load("best_FCNN_PC_model.pth"))
model.eval()
...
def joint_traj(trajectory,startangles,device,model):
    ...
    for point in trajectory[1:]:
        ...
        #input_tensor = torch.tensor([new_x, new_y], dtype=torch.float32).unsqueeze(0).to(device)
        new_theta1, new_theta2, new_theta3 = model(input_tensor).detach().cpu().numpy().squeeze()
        #new_x, new_y = FK(lengths, np.array([new_theta1, new_theta2, new_theta3]))
